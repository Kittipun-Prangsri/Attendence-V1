...
async function recordAttendance(type) {
    try {
        ...
        // ดึงข้อมูลผู้ใช้งานที่ล็อกอินอยู่ในระบบจาก localStorage
        const userStr = localStorage.getItem('user');
        if (userStr) {
            const user = JSON.parse(userStr);
            if (user && user.empId) {
                ...
                // ยืนยันการบันทึกโดยไม่ต้องพิมพ์รหัสพนักงานเอง
                const confirmMsg = `ยืนยันการบันทึกเวลา ${type === 'check-in' ? 'เข้างาน' : 'ออกงาน'} ของคุณ (${user.name})?`;
        ...
        // หากไม่มีการเข้าสู่ระบบ หรือหาไม่พบรหัส ให้ใช้กล่องป้อนข้อมูลแบบเดิม (Fallback)
        if (!empId) {
            empId = prompt("กรุณาระบุรหัสพนักงานของคุณ:");
            if (!empId) return;
        }
